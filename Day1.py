...
def sum_calibration_values(calibration_document):
    # Function to extract calibration values from the document
    def extract_calibration_value(line):
        # Find the first and last digits in the line
        digits = [char for char in line if char.isdigit()]

        # If only one digit is found, duplicate it to form a two-digit number
        if len(digits) == 1:
            return int(digits[0] * 2)
        # If more than one digit is found, combine the first and last digits
        elif len(digits) >= 2:
